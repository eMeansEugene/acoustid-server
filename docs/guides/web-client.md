...
- **`MediaRecorder`, а не `ScriptProcessorNode`.** `MediaRecorder` кодирует
  аудио в отдельном потоке браузера и не роняет сэмплы при нагрузке на
  основной поток (в отличие от устаревшего и снятого с поддержки
  `ScriptProcessorNode`).
- **`decodeAudioData()`.** Единственный штатный способ превратить сжатый
  WebM/Opus обратно в PCM в браузере — нет прямого доступа к сырым сэмплам
  во время записи `MediaRecorder`.
- **`OfflineAudioContext(1, …, 44100)`.** Частота дискретизации микрофона
  зависит от устройства и обычно не равна 44100 Гц, на которой обучен и
  настроен DSP-пайплайн сервера (`FftEngineConfig`, частотные полосы
  `PeakExtractor`). `OfflineAudioContext` с `numberOfChannels=1` и заданной
  `sampleRate` одновременно сводит в моно и ресемплирует за один проход
  рендеринга (`startRendering()`), без ручной реализации resampling.
- **WAV, а не сырой WebM.** `AudioDecoder` на сервере поддерживает только
  MP3 и WAV; `samplesToWav()` пишет минимальный 16-битный PCM WAV-заголовок
  вручную (44 байта) — простой формат, не требующий кодировщика.
