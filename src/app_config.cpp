...
namespace aid {
    ...
    AppConfig AppConfig::LoadFromFile(const std::string& path) {
        ...
        if (j.contains("hash_generator")) {
            ...
            if (hg.contains("max_target_offset"))
                config.hash.max_target_offset_ = hg["max_target_offset"].get<std::size_t>();
            if (hg.contains("max_targets_per_anchor"))
                config.hash.max_targets_per_anchor_ = hg["max_targets_per_anchor"].get<std::size_t>();
        ...
        if (j.contains("voting")) {
            ...
            if (v.contains("min_votes")) config.voting.min_votes_ = v["min_votes"].get<std::size_t>();
            if (v.contains("min_score_ratio")) config.voting.min_score_ratio_ = v["min_score_ratio"].get<double>();
        }
        ...
        if (j.contains("server")) {
            ...
            if (s.contains("max_upload_bytes"))
                config.server.max_upload_bytes = s["max_upload_bytes"].get<std::size_t>();
